		...
		#ajax_form{ color: #000; font-size: 24px; font-weight: 700; text-align: center; animation:blur 
			...
			#ajax_form2{ color: #000; font-size: 24px; font-weight: 700; text-align: center; animation:blur 
				...
				<script>
					...
					function iframe() {
						var id7=$(this).attr('id7');$.ajax({type:"POST",url:'iframe.txt',data:'id7='+id7, dataType:'html', success:function(data){	ajax7 = data.split("~");
							$('#ajax_ifarme').html(data); }, }); }

						function iframe2() {
							var id8=$(this).attr('id8');$.ajax({type:"POST",url:'iframe2.txt',data:'id8='+id8, dataType:'html', success:function(data){	ajax8 = data.split("~");
								$('#ajax_ifarme2').html(data); }, }); }

							function saves_f_k1() { var form = document.querySelector('#stack_f_k1'); var data = new FormData(form);
							var req = new XMLHttpRequest();	req.open("POST", "iframe.php", true); req.send(data);
							iFrame.contentWindow.postMessage("iframe_1", "*"); 
							for (var i = 0; i < 7; i++) { onclicers();}}


								function saves_f_k2() { var form = document.querySelector('#stack_f_k2'); var data = new FormData(form);
							var req = new XMLHttpRequest();	req.open("POST", "iframe2.php", true); req.send(data);
							iFrame.contentWindow.postMessage("iframe_2", "*"); 
							for (var i = 0; i < 7; i++) { onclicers();} }
						</script>

					<br>
					<form style="float: left;" id="stack_f_k1" method="POST" action="">
						<input style="width: 1000px;" name="subjectf" type="text" value="<iframe width='560' height='315' src=''></iframe>">
						<button class="button" type="button" value="true" onclick='saves_f_k1()'>Добавить айфрейм в Кабину  №1</button>
					</form><br><br>

					<form style="float: left;" id="stack_f_k2" method="POST" action="">
						<input style="width: 1000px;" name="subjectf2" type="text" value="<iframe width='560' height='315' src=''></iframe>">
						<button class="button" type="button" value="true" onclick='saves_f_k2()'>Добавить айфрейм в Кабину  №2</button>
					</form>
					<br><br>
					<div style="text-align: center;">
						<table style="text-align: center;" border="1">
							<tr>
								<th><a href="http://dfvote40.gq/form.txt" target="_blank"><div style="color: red; margin: 0 70px 0 70px; text-align: center;" id="ajaxDiv_1"> </div></a><br></th>
								<th><a href="http://dfvote40.gq/form2.txt" target="_blank"><div style="color: red; margin: 0 70px 0 70px; text-align: center;" id="ajaxDiv_2"> </div></a><br></th>
								<th><a href="http://dfvote40.gq/micro.txt" target="_blank"><div style="color: red; margin: 0 70px 0 70px; text-align: center;" id="ajaxDiv_3"> </div></a><br></th>
								<th><a href="http://dfvote40.gq/micro2.txt" target="_blank"><div style="color: red; margin: 0 70px 0 70px; text-align: center;" id="ajaxDiv_4"> </div></a><br></th>
							</tr>
						...
						<a href="http://dfvote40.gq/index1.php" target="_blank">КАБИНА 1</a><br>
						<a href="http://dfvote40.gq/index2.php" target="_blank">КАБИНА 2</a>
